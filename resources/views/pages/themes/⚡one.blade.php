...
<div x-data="masjidApp({
    ...
    <div class="absolute top-0 inset-x-0 h-28 bg-white/70 backdrop-blur grid grid-cols-4 rounded-b-[1.6rem]">
        <div class="flex items-center justify-center">
            <span class="text-7xl font-bold" x-text="time"></span>
        </div>
        <div class="col-span-2 flex-row flex justify-center items-center gap-2 border-x-2 border-yellow-600/40">
            <div class="px-4 py-1 flex flex-col justify-center items-center">
                @if($logo)
            ...
            <div class="flex px-2 flex-col justify-center items-center">
                <div class="text-3xl font-bold text-center">{{ $nama_masjid }}</div>
                <div class="text-lg text-center">{{ $alamat }}</div>
            </div>
        </div>
        <div class="flex flex-col justify-center text-center">
            <div class="text-2xl font-bold" x-text="hijri"></div>
            <div class="text-xl" x-text="tanggalMasehi"></div>
        </div>
    </div>
    ...
    <div x-show="mode === 'COUNTDOWN'" x-transition.opacity class="absolute bottom-16 inset-x-0 h-28 bg-white/60 backdrop-blur grid grid-cols-7 rounded-t-[1.6rem]">
        <template x-for="(p, i) in prayers" :key="p.key">
            <div class="flex flex-col items-center justify-center transition-all border-x border-yellow-600/50" :class="nextIndex === i ? 'bg-yellow-700 text-white rounded-t-[1.6rem]' : ''" >
                <span class="text-3xl" x-text="displayLabel(p)"></span>
                <span class="text-4xl font-bold" x-text="p.time"></span>
                <!-- COUNTDOWN -->
                <template x-if="nextIndex === i && countdown">
                    <span class="text-xl font-bold" x-text="countdown"></span>
                </template>
            </div>
        </template>
    </div>
    ...
    <div class="absolute bottom-0 inset-x-0 h-16 bg-gray-900 border-t-4 border-yellow-600 flex items-center overflow-hidden">
        <div class="z-20 text-xl bg-yellow-600 text-white px-6 h-full flex items-center font-bold shadow-2xl">
            INFO
        </div>
...
<script>
    Alpine.data('wallpaperRotator', ({ images, duration }) => ({
        ...
        init() {
            // console.log('durasi wallpaper:', this.duration);
