...
new class extends Component
{
    ...
    public $judulModal = 'Tambah Text Slider';

    #[Computed()]
    public function getTextSliders()
    {
        $texts = DB::table('text_sliders')
            ->orderBy('active', 'desc')
    ...
    {
        $this->dispatch('open-modal', name: 'modal_textslider');
    }

    public function edit($id)
    {
        $text = DB::table('text_sliders')->where('id', $id)->first();
        $this->id_text = $text->id;
        $this->text = $text->text;
        $this->isActive = $text->active;
        $this->editMode = true;
        $this->judulModal = 'Edit Text Slider';
        $this->dispatch('open-modal', name: 'modal_textslider');
    }
    ...
    {
        $orderIndex = DB::table('text_sliders')->max('order_index');
        return $orderIndex + 1;
    }

    public function setupTextSlider()
    {
        $this->validate([
            'text' => 'required',
            'isActive' => 'required'
        ...
        if($this->editMode) {
            DB::table('text_sliders')->where('id', $this->id_text)->update([
                'text' => $this->text,
                'active' => $this->isActive,
                'updated_at' => now()
            ]);
        } else {
            DB::table('text_sliders')->insert([
                'text' => $this->text,
    ...
    {
        $this->dispatch('close-modal', name: 'modal_textslider');
...
<div>
    ...
    <!--loader-->
    <div class="mb-6">
        <flux:heading size="xl">Setup Text Slider</flux:heading>
        <flux:text>Kelola text atau reminder yang akan ditampilkan secara slideshow di tengah halaman display.</flux:text>
    </div>
    ...
    <div class="overflow-x-auto border rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($this->getTextSliders as $item)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600">
                            ...
                            @endif
                            @if($item->order_index != count($this->getTextSliders))
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-10 text-center text-gray-500">
                            Data text slider belum tersedia.
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <x-custom-modal name="modal_textslider" width="w-2xl" :dismissible="false" :closable="false">
        <div class="mb-3">
            <h4 class="font-semibold text-lg">{{ $judulModal }}</h4>
            <flux:text>Anda dapat menambahkan/mengubah text slider dengan mengisi form berikut</flux:text>
        </div>
        <form wire:submit.prevent="setupTextSlider">
            <div class="mb-3">
